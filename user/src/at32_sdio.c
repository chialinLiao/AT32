...
//* I use the XT's SD board, the layout for SPI type, but also can use in the SDIO type 
//* should be care for that it can not use SDIO width mode(4bits), only use in 1 bit
//* because the layer not includes the d2,d3,d4 trace line. 
#define USE_SDIO_4_BIT  0 
...
sd_error_status_type sd_card_init(void)
{
  ...
  /* check power status */
  if(RESET == sdio_power_status_get(SDIOx))
  {
