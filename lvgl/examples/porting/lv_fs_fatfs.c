...
static lv_fs_res_t fs_open (lv_fs_drv_t * drv, void * file_p, const char * path, lv_fs_mode_t mode)
{
    //* convert the path for the fatfs format e.g. => "1:/file name"
    //* based on fatfs device id number
    char *path_buf = NULL;
    uint16_t path_len =strlen(path);

    path_buf = (char *)lv_mem_alloc(sizeof(char) * (path_len + 3));
    sprintf(path_buf, "1:/%s", path);
    ...
    FRESULT res = f_open(file_p, path_buf, flags);
